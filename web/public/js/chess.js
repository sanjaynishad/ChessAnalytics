...
var chess = (function() {
	...
	// system
	$el,
	oMovesList=$('<select/>'), sMovesList, sPGNHeader, flagHumanBlack, bReady = true, bAI = true, bCheck = false, bGameNotOver = true, lastStart = 0, lastEnd = 0, iHistPointr = -1, aHistory = [], kings = [0, 0], iRound = 1,
	...
	*/
	function writeHistory(bGraphRendrng, nStartPt, nEndPt, nPieceId, nTarget, nPromo) {
		...
		if(!colorFlag){
			if(nMoves === 0){
				oMovesList.append($('<option />').attr('value', nMoves).text('Game start'));
			}
			oMovesList.append($('<option />').attr('value', nMoves+1).text(String(nMoves + 1) + ". "));
		}else{
			oMovesList.find('option:last').text( oMovesList.find('option:last').text() + ' ' );
		}
		
		
		if ((nPieceId & 7) === 2 && (nEndPt - nStartPt + 2 | 4) === 4) { 
			var str = "O-O" + (nStartPt - nEndPt === 2 ? "-O" : "");
			sMovesList += str; 
			oMovesList.find('option:last').text( oMovesList.find('option:last').text() + str );
		}
		else { 
			var str = ((nPieceId & 7) !== 1 ? "KNBRQ".charAt(nPieceId - 2 & 7) : "") + (((nPieceId & 7) === 1 && bWriteCapture) || disambiguateX ? String.fromCharCode(96 + nStartPt % 10) : "") + (disambiguateY ? String(nStartPosY + 1) : "") + (bWriteCapture ? "x" : "") + String.fromCharCode(96 + nEndPt % 10) + String(nEndPosY + 1) + sPromoAlg + (bCheck ? "+" : ""); 
			sMovesList += str;
			oMovesList.find('option:last').text( oMovesList.find('option:last').text() + str );
		}
		
		$(oMovesSelect).html(oMovesList.html());
	...
	function writeFlatPieces() {
		...
		for (var iCell = 0; iCell < 64; iCell++) {
			...
			if (nSquareId === lastStart || nSquareId === lastEnd) { 
				//oSquareCell.style.backgroundColor = (nSquareId * 11 - nSquareId % 10) / 10 & 1 ? "#c0a1a1" : "#e8c9c9"; } else { oSquareCell.style.backgroundColor = ""; 
			}
		}
